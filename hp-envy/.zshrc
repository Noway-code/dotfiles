...
alias swaycon="nano ~/.config/sway/config"
alias config="nautilus ~/.config/"
alias alaccon="nano ~/.config/alacritty/alacritty.toml"

# Enhanced 'cd' function using fzf with strict matching
fcd() {
  local dir
  if command -v fd >/dev/null 2>&1; then
    dir=$(fd --type d --exclude .git | fzf --exact +m)
  else
    dir=$(find ${1:-.} -type d -print 2> /dev/null | fzf --exact +m)
  fi
  if [[ -n "$dir" ]]; then
    cd "$dir"
    zle reset-prompt
...
# Create a Zle widget for the fcd function
zle -N fcd
